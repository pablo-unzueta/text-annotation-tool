...
from .models import DocumentCreate, DocumentResponse, AnnotationCreate, AnnotationResponse, AnnotationFilter
...
def create_annotation(annotation: AnnotationCreate, db: Session = Depends(get_db)):
    ...
    document = db.query(models.Document).filter(models.Document.id == annotation.document_id).first()
    if document is None:
        raise HTTPException(status_code=404, detail="Document not found")
...
def update_annotation(annotation_id: int, annotation: AnnotationCreate, db: Session = Depends(get_db)):
